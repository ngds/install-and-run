...
if [ ! -f /var/lib/pgsql/9.1/data/PG_VERSION ]; then
  ...
  sudo -u postgres psql -d ckan_default -f /usr/pgsql-9.1/share/contrib/postgis-1.5/postgis.sql
  sudo -u postgres psql -d ckan_default -f /usr/pgsql-9.1/share/contrib/postgis-1.5/spatial_ref_sys.sql
  sudo -u postgres psql -d ckan_default -c 'GRANT SELECT, UPDATE, INSERT, DELETE ON spatial_ref_sys TO ckan_default'
  sudo -u postgres psql -d ckan_default -c 'GRANT SELECT, UPDATE, INSERT, DELETE ON geometry_columns TO ckan_default'
  sudo -u postgres psql postgres postgres -c "select pg_reload_conf();"
