...
function configure_properties() {
    ...
    #CKAN Site URL
    site_url='http://54.197.43.115'

    #Application Deployment setup(central|node).
    deployment_type="node"

    # CKAN DB username
    pg_id_for_ckan=node_ckan_user
    # CKAN DB password
    pg_pw_for_ckan=pass
    #CKAN DB name
    pg_db_for_ckan=node_ckan_db
    #Datastore DB Username
    pg_id_datastore=node_datastore_user
    #Datastore DB pass
    pg_pw_datastore=pass
    #Datastore Database Name
    pg_db_for_datastore=node_datastore
    # PyCSW DB username
    pg_id_for_pycsw=aggregator_ckan_user
    # PyCSW DB pass
    pg_pw_for_pycsw=pass
    #PyCSW Database Name
    pg_db_for_pycsw=pycsw_ckan_aggregator
...
function setup_env() {
    ...
    SMTP_SERVER="mail.geothermaldata.org:587"
